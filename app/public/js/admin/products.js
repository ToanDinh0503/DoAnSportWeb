document.addEventListener("DOMContentLoaded", function () {
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(
          "/WebbandoTT/app/api/products/delete.php",
          {
            method: "POST",
            headers: {
  ...
  async function updateProducts(
    ...
    try {
      const response = await fetch(
        `/WebbandoTT/app/api/products/search.php?search=${searchTerm}&category=${categoryId}&brand=${brandId}`
      );
      const data = await response.json();
            ...
            .map(
              (product) => `
                    <tr>
                        <td>
                            <div class="product-info-cell">
                                <div class="product-img-wrapper">
                                    <img src="${
                                      product.hinh_anh
                                        ? "/WebbandoTT/public/uploads/products/" +
                                          product.hinh_anh
                        ...
                        <td>
                            <div class="fw-semibold">${new Intl.NumberFormat(
                              "vi-VN"
                            ).format(product.gia)}₫</div>
                            ${
                              product.gia_khuyen_mai
                                ? `<div class="text-danger small">${new Intl.NumberFormat(
                                    "vi-VN"
                                  ).format(product.gia_khuyen_mai)}₫</div>`
                                : ""
                            }
                        </td>
                        ...
                        <td>
                            <div class="action-buttons">
                                <a href="/WebbandoTT/admin/products/edit?id=${
                                  product.id
                                }" 
                                   class="btn-action" 
                                   title="Sửa">
  ...
  searchInput.addEventListener(
    "input",
    debounce(function (e) {
      updateProducts(e.target.value, categoryFilter.value, brandFilter.value);
    }, 300)
  );

  categoryFilter.addEventListener("change", function () {
    updateProducts(searchInput.value, this.value, brandFilter.value);
  });

  brandFilter.addEventListener("change", function () {
    updateProducts(searchInput.value, categoryFilter.value, this.value);
  });
